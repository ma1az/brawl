...
function frames_keySwitch ( key, state )
	...
	elseif key == KEYS.CONFIRM then -- Enter
			...
			checkWindow = guiCreateWindow(x,y,width,height,"Confirmation Window",false)
			checkLabel = guiCreateLabel(8,25,width,height,"A texture will be applied, and this action cannot be undone. Are you sure?",false,checkWindow)
	
			guiWindowSetMovable(checkWindow,false)
			guiWindowSetSizable(checkWindow,false)
			guiSetInputEnabled(true)

			okButton = guiCreateButton(5,80,160,30,"Accept ("..price.."$)",false,checkWindow)
			closeButton = guiCreateButton(175,80,160,30,"Cancel",false,checkWindow)
				...
				if b == "left" then	
					...
					elseif source == okButton then
						if exports["global"]:hasMoney(localPlayer,price) then
							...
							outputChatBox("[!]#ffffff You don't have enough money!",255,0,0,true)
						end
					end
				end
			end)
