	...
	function(id)
		...
		if global:hasItem(client, 4, interior) or global:hasItem(client, 5, interior) or (integration:isPlayerAdmin(client) and global:isAdminOnDuty(client)) or (interior==0) then
			...
			if not data or not data[id] then
				...
				if query then
					outputChatBox("[!]#ffffff The texture has been successfully removed. (ID #" .. id .. ").", client, 0, 255, 0,true)
			...
			outputChatBox("[!]#ffffff You cannot perform this action without the key!", client, 255, 0, 0,true)
		end
	end
)
...
function newTexture(source, url, texture,price)
	...
	if dimension >= 1 or (integration:isPlayerLeadAdmin(source) and global:isAdminOnDuty(source)) or integration:isPlayerScripter(source) then
		if global:hasItem(source, 4, dimension) or global:hasItem(source, 5, dimension) or (integration:isPlayerAdmin(source) and global:isAdminOnDuty(source)) or (dimension==0) then
			if savedTextures[dimension] then
				for k, v in pairs(savedTextures[dimension]) do
					if v.texture:lower() == texture:lower() then
						outputChatBox ( "[!]#ffffff There is already a texture in the area where you want to apply a texture. To remove it, use '/wallpaper'.", source, 255, 0, 0,true )
						return false
					end
				end
			end
				...
				function(qh)
					...
					if rows > 0 then
						...
						if id then
							...
							outputChatBox ( "[!]#ffffff The texture has been successfully applied. To remove it, use '/wallpaper'.", source, 0, 255, 0,true )
							return true
						end
					end
				end,
			...
			outputChatBox("[!]#ffffff You are not the owner of this property!", source, 255, 0, 0, true)
			return false
		end
	else
		outputChatBox("[!]#ffffff A texture cannot be applied to this area!", source, 255, 0, 0, true)
		return false
	end
	return false
end
