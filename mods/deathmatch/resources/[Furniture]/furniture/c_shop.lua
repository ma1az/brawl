...
function drawnShop()
	...
	dxDrawText(" Categories and Help",x+w+43,y+4,170-8+(x+w+43),20+(y+4),tocolor(255,255,255),1,font_small,"left","center",false,false,false,true)
	...
	if key >= 0 then
		...
		if not furnitures[selectedCategory][key].isRenderable then text = " No picture" else if furnitures[selectedCategory][key].isModding then text = "" else text = "" end end
		if furnitures[selectedCategory][key].isModding then text2 = " mod object" else text2 = "" end
		dxDrawText(" Furniture Name: "..furnitures[selectedCategory][key].name.."\n "..furnitures[selectedCategory][key].price.."\n"..text..text2,x+w/2-250/2,y+h-86,250+(x+w/2-250/2),40+(y+h-86),tocolor(255,255,255),1,font,"center","bottom")
	end
